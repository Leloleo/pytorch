...
import torch
from torch._inductor.utils import argsort_sym
...
def collect_metadata_from_dynamo_fx(
    ...
    *,
    ...
    def inner(*flat_args):
        ...
        fake_mode_is_none = fake_mode is None
        fake_mode = contextlib.nullcontext() if fake_mode_is_none else fake_mode
        mode = fake_mode
        suppress_pending = contextlib.nullcontext()
        if not fake_mode_is_none and (shape_env := fake_mode.shape_env):
            suppress_pending = shape_env.ignore_fresh_unbacked_symbols()

        with disable_above, fake_mode, suppress_pending:
            flat_outs = fx(*flat_args)
            if not fake_mode_is_none:
                fake_mode.epoch += 1
        ...
        for arg in flat_args:
            ...
            # !!! TODO: Update to Faketensor so that we can track these things
            mutates_data = has_data_mutation(arg)
            mutates_metadata = has_metadata_mutation(
                arg, arg, check_only_storage_mutation=False
            )
            ...
            mutates_storage_metadata = has_metadata_mutation(
                arg, arg, check_only_storage_mutation=True
            )
            ...
            mutations_under_no_grad_or_inference_mode = (
                mutates_data
                and are_all_mutations_under_no_grad_or_inference_mode(arg)
            )
            mutation_inductor_storage_resize = was_inductor_storage_resized(arg)

            if mutates_storage_metadata:
                mutates_data = False

            requires_grad = isinstance(arg, torch.Tensor) and arg.requires_grad
        ...
        out_tensor_ids = {id(o): i for i, o in enumerate(flat_outs)}
        ...
        for o in flat_outs:
            if isinstance(o, torch.Tensor):
                ...
                # !!! TODO: Update this, as is not functional so won't be tracked
        ...
        # NOTE: Why must this be a seperate loop? Do we need to populate out_storage_to_metadata dict first?
        for o in flat_outs:
            # !!! TODO: Update this, as is not functional so won't be tracked
            ...
            grad_fn = None
            # !!! TODO: No longer functional so we need to reevaluate this
...
from a multi-output view call"
            ...
            if (
                ...
                # !!! TODO: Change this to functional Tensor check
                from torch._subclasses.fake_tensor import FakeTensor
                if isinstance(o, FakeTensor):
                    fake_tensor = FunctionalTensorMetadataEq(o)
        ...
        # TODO: Fix this config as no longer using functorch here
        ...
        f_input_tangents = [
            ...
            for inp, info in zip(flat_args, input_info)
        ...
        f_output_tangents = [
            o
            for o, info in zip(flat_outs, output_info)
            if info.output_type
            in [
        ...
        # NOTE: Since no functional this should be fine
        ...
        f_metadata_mutated_inputs = [
            inp for inp, info in zip(flat_args, input_info) if info.mutates_metadata
        ]
        ...
        f_fw_graph_outs = list(flat_outs)
