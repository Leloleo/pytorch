...
TensorList, SymInt, SymIntList and CustomObg. Values are uniquely identifiable
...
buffers, which is extremely important in memory-bound services like Adfinder,
Adranker, and Adretriever. That is, if two tensors don’t have an overlapping
lifetime during execution, and the first tensor is larger than the second, then
the second tensor can share the same chunk of memory as the first. As such, the
main goal of our planning mechanism is to pack tensors efficiently in memory
with minimal impact on E2E execution latency.
